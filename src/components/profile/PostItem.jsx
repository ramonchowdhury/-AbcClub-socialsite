...
import { deletePost, addLikeByHandle, removeLikeByHandle } from '../../actions/postActions'; 
...
class PostItem extends Component{
    ...
    handleLike(id, handle){
        this.props.addLikeByHandle(id, handle);
    }
    handleUnlike(id, handle){   
        this.props.removeLikeByHandle(id, handle);
    }
    findUserLike(likes) {
    ...
    render() {
        const { post, auth, handle } = this.props;
        return (
                        ...
                        <ul className="nav nav-pills nav-pills-custom">
                            {
                                this.findUserLike(post.likes) === true ?  (
                                    <button type="button" onClick={this.handleUnlike.bind(this, post._id, handle)} className="btn btn-light mr-1">
                                        <i className={classnames('text-secondary fas fa-thumbs-up', {'text-info': this.findUserLike(post.likes)})}></i>
                                        <span className="badge badge-light">{post.likes.length}</span>
                                    </button>
                                ) :
                                (
                                    <button type="button" onClick={this.handleLike.bind(this, post._id, handle)} className="btn btn-light mr-1">
...
PostItem.propTypes = {
    ...
    addLikeByHandle: PropTypes.func.isRequired,
    removeLikeByHandle: PropTypes.func.isRequired,
...
export default connect(mapStateToProps, { deletePost, addLikeByHandle, removeLikeByHandle })(PostItem);